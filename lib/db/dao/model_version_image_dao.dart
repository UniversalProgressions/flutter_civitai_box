...
final class ModelVersionImageDao {
  const ModelVersionImageDao();
  ...
  Future<Map<String, Object?>?> getById(int id) => _db.then(
    (db) => db
        .query('model_version_image', where: 'id = ?', whereArgs: [id])
  ...
  Future<List<Map<String, Object?>>> getAll() =>
      _db.then((db) => db.query('model_version_image'));

  Future<int> delete(int id) => _db.then(
    (db) => db.delete('model_version_image', where: 'id = ?', whereArgs: [id]),
  );
