...
**Indexes:** `idx_user_custom_preview_model` on (`model_id`), `idx_user_custom_preview_version` on (`model_version_id`).
...
**Indexes:** `idx_user_custom_tag_version` on (`model_version_id`), `idx_user_custom_tag_name` on (`tag_name`).
